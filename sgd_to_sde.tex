...
%------------------------------------------------------------
% 1-sample gradient noise covariance for a two–layer linear net
%------------------------------------------------------------
...
\end{itemize}
Most interesting to look into are: low rank solutions, learning more important features first, edge of stability.

\subsection{Experiments to show low rank bias of GD}



\section{Implicit biases of SGD on DLNs}
DLNs are interesting to study as mathematical tractable models of deep learning. It seems natural to study the implicit biases of SGD on DLNs as a first step toward understanding the implicit biases introduced by the gradient and the noise. A particularly interesting phenomena is that SGD shows an implicit bias towards low rank solutions. In particular this effects seems different in SGD compared to GD where the anisotropy of the noise induces further bias toward low rank solutions. This is something that we could check empirically on DLNs. 

Another interesting aspect of DLNs is their saddle to saddle dynamics. There is a large fraction of non-strict saddle points in DLNs (i.e. such that the Hessian can have a zero eigenvalue). We could study the escape times and mixing times of GD, SGD and SDEs around saddle points. Since we know about some characterizations of theses saddle points, we could look into different SDE models around these saddle points.
